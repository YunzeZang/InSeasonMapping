...
class ICS:
    ...
    """
    def __init__(self, prioCM, sampleNumber=6000, binWidth=0.05, balancedNumber = 100000):
    ...
    def estimate(self, y_predicted_p_test,y_predicted_n_test,y_predicted_c,binWidth = 0.05):
        ...
        raw_trusted_size = y_predicted_p_test.shape[0]
        ...
        # if y_predicted_p_test.shape[0]>20000:
        ...
        # if raw_trusted_size<20000: # if the number of trusted samples is small, ajust the proportion of classified samples
        #     size_t_p = raw_trusted_size/2
        #     size_t_n = raw_trusted_size/2
        #     size_t_c = y_predicted_classified.shape[0]

        #     RSP_t_p = size_t_p/(size_t_p+size_t_n+size_t_c)*self.RSP_t_p
        #     RSP_t_n = size_t_n/(size_t_p+size_t_n+size_t_c)*self.RSP_t_n
        #     RSP_c = size_t_c/(size_t_p+size_t_n+size_t_c)*self.RSP_c

        #     self.RSP_t_p = RSP_t_p/(RSP_t_p+RSP_t_n+RSP_c_p).sum()
        #     self.RSP_t_n = RSP_t_n/(RSP_t_p+RSP_t_n+RSP_c_n).sum()
        #     self.RSP_c = RSP_c/(RSP_t_p+RSP_t_n+RSP_c).sum()
    ...
    def deriveP(self,N_Yt_p,N_Yt_n,Y):
        ...
        for i in range(0,hist.shape[0]-1):
            ...
            if (N_Yt_p_Bi.shape[0]+N_Yt_n_Bi.shape[0])==0:
                P_Yt_1_given_gX = 0
            else:
